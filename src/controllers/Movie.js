...
exports.updateMovie = async (req, res) => {
	try {
		...
		const updatedMovie = await Movie.findOneAndUpdate(query, newMovie, {
			new: true,
		});
	...
	try {
		...
		if (!req.body.users) {
			res
				.status(200)
				.send(
					user1.acceptedMovies.length > 0
						? user1.acceptedMovies
						: "you didn't accepted any movie!"
				);
		} else {
			req.body.users.forEach(async (user, index, arr) => {
				...
				allAcceptedMovies =
					(await tempUserMovies) && allAcceptedMovies.concat(tempUserMovies);
					...
					res
						...
						.send(
							matchedMovies.length > 0
								? matchedMovies
								: "sorry you didnt matched on any movies :("
						);
				}
			});
		}
